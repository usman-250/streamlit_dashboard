...
def main_fun(job_description,jd_select_box):
    try:
        json_path = './scrapped_json/result.json'
        resumes_path = './testing_resume/*.*'
        tree_sheet_path = './tree_sheet/Tree.xlsx'
    ...
    #                 terms = ['EXT','NEU','AGR', 'CON', 'OPN']
    #                 predictions = zip(predictions,terms)
    #                 predictions = [i[1] for i in predictions if i[0]==1]
                else:
    ...
    #                 predictions = [i[1] for i in predictions if i[0]==1]
            ...
            #         print(data[key]['name'],predictions)

            except:
                profiles_info[key]['activity_similar'] = 0
                profiles_info[key]['personality'] = []
                pass
        with open('./dashboard_json/'+jd_select_box+'.json', 'w') as fp:
